...
const productosBase = [
  {
    ...
    precio: 45.00,
    descuento: 15,
    img: '../../img/uploads/sg-product-7790001000010-gold-standard-100-whey.svg',
  ...
  },
  {
    ...
    precio: 22.50,
    descuento: 0,
    img: '../../img/uploads/sg-product-7790001000027-platinum-creatine-400g.svg',
    categoria: 'Suplementos',
  ...
  },
  {
    ...
    precio: 30.00,
    descuento: 0,
    img: '../../img/uploads/sg-product-7790001000034-original-bcaa-fresa-kiwi.svg',
    categoria: 'Suplementos',
    sucursal: 'SquatGym Central',
    stock: 0
  },
  {
    ...
    precio: 28.99,
    descuento: 0,
    img: '../../img/uploads/sg-product-7790001000041-c4-original-pre-workout.svg',
    categoria: 'Suplementos',
    sucursal: 'Sucursal Sur',
    stock: 12
  },
  {
    ...
    precio: 12.50,
    descuento: 0,
    img: '../../img/uploads/sg-product-7790002000019-shaker-squatgym-pro-700ml.svg',
    icono: 'water_bottle',
    categoria: 'Bebidas',
    sucursal: 'SquatGym Central',
    stock: 36
...
productosBase.forEach((producto) => {
  producto.img = obtenerImagenProductoLocal(producto);
  producto.imgNombre = obtenerNombreImagenProductoLocal(producto);
});
...
async function guardarImagenPendienteKiosco(producto) {
...
async function procesarArchivoImagenProducto(file) {
  ...
  try {
    ...
    const currentSrc = document.getElementById('new-product-image')?.value || '';
    const currentName = document.getElementById('new-product-image-name')?.value || '';
    const fileName = document.getElementById('product-image-name');
    ...
    asignarValorProducto('new-product-image', currentSrc);
    asignarValorProducto('new-product-image-name', currentName);

    if (fileName) {
      fileName.textContent = `Vista previa: ${file.name}. Se guardara al confirmar.`;
...
async function agregarProductoPersonalizado(event) {
  ...
  let img = document.getElementById('new-product-image')?.value.trim() || '';
  let imgNombre = document.getElementById('new-product-image-name')?.value.trim() || '';
  ...
  if (!nombre || !categoria || !Number.isFinite(precio) || precio <= 0) {
    mostrarErrorProducto('Completa nombre, categoria y precio.');
    return;
  }

  if (!Number.isFinite(stock) || stock < 0) {
    mostrarErrorProducto('La cantidad debe ser un numero valido.');
    return;
  }
  ...
  const productoId = productoExistente?.id || Date.now();
  let productoDraft = {
    id: productoId,
    nombre,
    marca,
    codigoBarras: productoExistente?.codigoBarras || `KIO-${productoId}`,
    categoria,
  ...
  try {
    ...
    productoDraft = await guardarImagenPendienteKiosco(productoDraft);
    img = productoDraft.img;
    imgNombre = productoDraft.imgNombre;
  } catch (error) {
...
function limpiarModoImpresionCaja() {
  document.body.classList.remove('cash-print-mode');
}

function imprimirComprobanteCaja() {
  document.body.classList.add('cash-print-mode');
...
function construirListadoCierreCaja(ventas, resumen, datosCierre) {
  ...
  return `
    <section class="cash-close-report">
      ...
      <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; margin-bottom: 18px;">
        ...
        <div><strong>IVA 21%</strong><br>${formatCurrency(resumen.impuestos)}</div>
...
window.addEventListener('afterprint', limpiarModoImpresionCaja);
